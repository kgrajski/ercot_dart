"""ERCOT Base Client Module

This module provides the ERCOTBaseClient class, which serves as the foundation for all ERCOT API clients.
It handles authentication, data fetching, pagination, and basic data processing.

Key Features:
- OAuth 2.0 authentication with ERCOT's APIs
- Automatic pagination handling
- Rate limiting (60 requests per minute)
- Automatic CSV and database saving
- UTC and local timestamp conversion
- Progress bars for long downloads

For endpoints with sub-hourly data (e.g., RT SPP's 15-minute intervals):
- Base functionality works at the hourly level
- Sub-hourly interval information is preserved
...
class ERCOTBaseClient:
    ...
    def estimate_download_time(self, total_pages: int) -> str:
        ...
        total_estimated_seconds = time_per_page * (
            total_pages - 1
    ...
    def print_pagination_info(
        self, total_records: int, records_per_page: int, total_pages: int
    ) -> str:
    ...
    def _build_query_params(self, current_date: datetime, params: Dict) -> Dict:
        ...
        hour_ending = params.get(
            "postedHourEnding", "6:00"
        )  # Default to 6 AM if not specified
        hours_before = params.get(
            "hours_before", 1
        ...
        try:
            ...
            raise ValueError(
                f"Invalid hour_ending format. Expected 'H:mm' (e.g., '6:00'), got '{hour_ending}'"
            ) from e
        ...
        current_params = {
            ...
            if k not in ["postedHourEnding", "hours_before"]
        }
    ...
    def _save_data(
        self, df: pd.DataFrame, endpoint_key: str, params: Dict
    ) -> pd.DataFrame:
    ...
    def get_data(
        ...
        if df is None:
            ...
            date_from = params.get("postedDatetimeFrom") or params.get(
                "deliveryDateFrom"
            )
            ...
            if not date_from or not date_to:
                raise ValueError(
                    "Either postedDatetime or deliveryDate range parameters are required"
                )
            ...
            with tqdm(
                total=num_days, desc="Processing days", unit="day", position=0
            ) as day_pbar:
                # Process each day
                for current_date in date_range:
                    ...
                    day_pbar.set_postfix_str(
                        f"date={current_date.strftime('%Y-%m-%d')} | "
                        + self.print_pagination_info(
                            total_records, records_per_page, total_pages
                        )
                    )
