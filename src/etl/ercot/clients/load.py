...
class LoadForecastETL(ERCOTBaseETL):
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_clean = df_melted.rename(columns={"DSTFlag": "dst_flag"})
    ...
    def validate_data(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if (
                df[["posted_datetime", "utc_ts", "location", "load_forecast"]]
                .isnull()
                .any()
                .any()
            ):
            ...
            invalid_locations = df[~df["location"].isin(self.WEATHER_ZONES)][
                "location"
            ].unique()
