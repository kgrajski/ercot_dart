...
class DartSltExp2Dataset(StudyDataset):
    """PyTorch Dataset for ERCOT DART SLT Experiment 2.

    Simple dataset loader for final exp2 datasets. All categorical encoding
    ...
    # Class-level experiment identifier
    exp_name = "exp2"
    ...
    def __init__(
        ...
        fnames = [
            f
            for f in os.listdir(spp_dir)
            if "final_dataset" in f and self.exp_name in f
        ]
        if not fnames:
        ...
        super().__init__(experiment_id=self.exp_name, output_dir=output_dir)
    ...
    def _get_default_features(self):
        ...
        - Raw categorical features (redundant with ETF encoding)
        - Z-score standardized versions (columns ending with '_z')
        """
        # Get all column names
        all_columns = self.df.columns.tolist()

        # Exclude metadata, target, raw categoricals, and z-score columns
        excluded_patterns = [
            # Metadata columns
            "utc_ts",
            "local_ts",
        ...
        # Enhanced Temporal Features (ETF) - new in exp2
        etf_features = [col for col in candidate_columns if col.startswith("etf_")]
        ...
        print(f"  ETF features ({len(etf_features)}): {etf_features}")
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            dict: Sample containing 'features', 'target', and 'metadata'
        """
        # Get the row
