"""Base Model Module for Experiment 1.

This module provides the BaseExp1Model class, which serves as the foundation for all 
Experiment 1 model implementations. It handles common functionality including:
...
from src.models.ercot.exp1.bootstrap_evaluator import BootstrapEvaluator
from src.models.ercot.exp1.evaluation_metrics import EvaluationMetrics


class BaseExp1Model(ABC):
    """Abstract base class for Experiment 1 models.

    This class provides common functionality for DART price prediction models,
    ...
    def load_data(self, dataset) -> None:
        """Load and split data from DartSltExp1Dataset with optional feature filtering and scaling.
    ...
    def _save_results(self) -> None:
        ...
        for hour, hour_results in self.results.items():
            # Create a clean copy handling list-type fields appropriately
            clean_results = {}
    ...
    def _save_coefficients(self) -> None:
        """Save model coefficients in analysis-friendly CSV format."""
        if not self.results:
            return

        # Collect all coefficients with feature names
        coeff_data = []
                    ...
                    coeff_data.append(
                        {
                            ...
                            "is_zero": coeff == 0,  # Useful for Lasso
        ...
        if coeff_data:
            ...
            coeff_filename = f"coefficients_{self.model_type}.csv"
            coeff_df.to_csv(os.path.join(self.model_dir, coeff_filename), index=False)
                ...
                .agg(
                    {
                        ...
                        "is_zero": "sum",  # How many hours this feature was zeroed
                    }
                )
                .round(6)
            )
            ...
            print(f"Coefficients saved: {coeff_filename}")
            ...
            print(
                f"Features: {n_features} total, {n_zero_features} had zero coefficients in some hours"
            )
    ...
    def _get_model_abbreviation(self):
        ...
        abbreviations = {
            ...
            "ridge_regression_alpha_10.0": "ridge",
            "lasso_regression_alpha_0.01": "lasso",
        }
