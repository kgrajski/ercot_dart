"""Bootstrap Evaluation Module for ERCOT DART Price Prediction.

This module provides the BootstrapEvaluator class which handles bootstrap resampling
evaluation with comprehensive metrics. This separates evaluation logic from model
training logic for better architecture and maintainability.

Key Features:
- Bootstrap resampling with out-of-bag (OOB) evaluation
- Comprehensive metrics via EvaluationMetrics integration
...
class BootstrapEvaluator:
    ...
    def _evaluate_bootstrap_model(
        ...
        try:
            ...
            # Quick R² calculation for progress reporting
            score = r2_score(y_test, y_pred)
            print(f"    Bootstrap {iteration}: R² = {score:.4f}")
    ...
    def _calculate_bootstrap_metrics(
        ...
        try:
            ...
            result = {
                **flattened_bootstrap,
                "bootstrap_r2_mean": bootstrap_summary.get("r2", {}).get(
                    "mean", np.nan
                ),
                "bootstrap_r2_std": bootstrap_summary.get("r2", {}).get("std", np.nan),
                "n_train_samples": n_train_samples,
                "bootstrap_iterations": bootstrap_iterations,
                "failed_iterations": failed_iterations,
